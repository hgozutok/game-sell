    ...
    for (const externalProduct of availableProducts) {
      try {
        ...
        if (productData.genres && Array.isArray(productData.genres) && productData.genres.length > 0) {
          if (processed === 0) logger.info(`📚 Adding ${productData.genres.length} genres as collections`)
        ...
        if (productData.platform) {
          if (processed === 0) logger.info(`🎮 Adding platform as collection: ${productData.platform}`)
        ...
        // Add provider as tag
        if (processed === 0) logger.info(`🏷️ Adding provider tag: ${provider}`)
        ...
        if (productData.tags && Array.isArray(productData.tags) && productData.tags.length > 0) {
          if (processed === 0) logger.info(`🏷️ Adding ${productData.tags.length} custom tags`)
        ...
        if (existingProducts && existingProducts.length > 0) {
          ...
          if (importedProducts.length === 0) {
            logger.info(`✨ Creating first product: ${productName}`)
